...
async function main() {
    try {
        ...
        for (let user of userCookie) {
            ...
            if ($.ckStatus) {
                // 抽奖签到
                const lotterySignResult = await lotterySignin(user)

                if (lotterySignResult[0] > 0) {
                    for (let i = 0; i < lotterySignResult[0]; i++) {
                        // 抽奖
                        await new Promise(r => setTimeout(r, 2000));
                        await lotteryClock(user, lotterySignResult[1]);
                    }
                }
                //查询用户信息
...
async function getLotteryActivityInfo(user) {
    try {
        ...
        if (res?.code === '0000' && res?.data) {
            const infoStr = res.data.info;
            if (!infoStr) {
                $.log(`⛔️ API响应中缺少 'info' 字段`);
                return null;
            }

            let infoData;
            try {
                infoData = $.toObj(infoStr);
            } catch (e) {
                $.log(`⛔️ 解析 'info' 字段 (JSON字符串) 失败: ${e}`);
                return null;
            }

            const activityNo = res.data.activity_no;
            let componentNo = null;

            for (const component of infoData?.list || []) {
                if (component.comName === 'turntablecom') {
                    componentNo = component.data?.component_no;
                    break;
                }
            }

            if (activityNo && componentNo) {
                $.log(`🎉 成功获取抽奖活动ID: activity_no=${activityNo}, component_no=${componentNo}`);
                return {activity_no: activityNo, component_no: componentNo};
            }

            $.log(`⛔️ 未找到 'turntablecom' 组件或相关ID (activity_no: ${activityNo}, component_no: ${componentNo})`);
            return null;
        } else {
            $.log(`⛔️ 获取抽奖活动页面信息失败: ${res?.message || '未知错误'}`);
            return null;
        }
    } catch (e) {
        $.log(`⛔️ 获取抽奖活动信息时发生异常: ${e}`);
        return null;
    }
}
...
async function lotterySignin(user) {
    try {
        ...
        const activityInfo = await getLotteryActivityInfo(user)
        if (!activityInfo) {
            $.log(`⛔️ 抽奖签到: 获取抽奖活动ID失败`);
            return [0, null];
        }
        ...
        const opts = {
            ...
            dataType: "json",
            body: {
                'component_no': activityInfo.component_no,
                'activity_no': activityInfo.activity_no
            }
        }
        let res = await fetch(opts);
        console.log(res)
        const chance = res?.data?.chance || 0;
        const status = res?.code === '0000' ?
            `✅ 抽奖签到: 成功, 获得${chance}次抽奖机会` :
            `⛔️ 抽奖签到: ${res?.message || '未知错误'}`;
        console.log(status)
        $.log(`${status}\n`);
        return [chance, activityInfo];
    } catch (e) {
        $.log(`⛔️ 抽奖签到失败！${e}\n`)
        return [0, null];
    }
...
// 抽奖
async function lotteryClock(user, activityInfo) {
    try {
        const opts = {
...
/** ---------------------------------固定不动区域----------------------------------------- */
//prettier-ignore
async function sendMsg(a) {
    a && ($.isNode() ? await notify.sendNotify($.name, a) : $.msg($.name, $.title || "", a, {"media-url": $.avatar}))
}
...
async function Request(t) {
    ...
    try {
        ...
        const y = s && "form" == n ? $.queryStr(s) : $.toStr(s), l = {
                ...t, ...t?.opts ? t.opts : {},
                url: c,
                headers: r, ..."post" === p && {body: y}, ..."get" === p && a && {params: a},
                timeout: i
            },
